...
public class AdjacencyListGraph<T> {
	...
	public Map<T, Integer> shortestPath(T startVertex, T endVertex) {
		...
		Map<T, T> shortestDistanceSource = new HashMap<>();
		...
		shortestDistanceSource.put(startVertex, startVertex)
		...
		while (!queue.isEmpty()) {
			...
			for (Map.Entry<T, Integer> neighborEntry : adjacencyList.get(current).entrySet()) {
				...
				if (newDist < distances.get(neighbor)) {
					...
					shortestDistanceSource.put(neighbor, current);
		...
		while(current!=startVertex) {
			path.push(current);
			current = shortestDistanceSource.get(current);
		}
